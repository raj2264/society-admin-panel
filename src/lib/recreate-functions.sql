...
DROP FUNCTION IF EXISTS generate_maintenance_bills(UUID, UUID, DATE, DATE, JSONB, DECIMAL, BOOLEAN);
DROP FUNCTION IF EXISTS generate_bill_pdf(UUID);
DROP FUNCTION IF EXISTS send_bill_to_resident(UUID);
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Create the bill
      BEGIN
        -- Set debug info for bill creation
        v_debug_info := format('Creating bill with bill_date: %s, issue_date: %s', 
          p_bill_date, p_bill_date);
        
        -- Ensure bill_date is not null
        IF p_bill_date IS NULL THEN
          v_error_message := 'Bill date cannot be null';
          RAISE EXCEPTION '%', v_error_message;
        END IF;

        -- Create the bill and capture the returned ID
        WITH inserted_bill AS (
          INSERT INTO maintenance_bills (
            ...
            COALESCE(p_bill_date, CURRENT_DATE)  -- Use COALESCE to ensure we never insert NULL
          ) RETURNING id
        )
        SELECT id INTO v_bill_id FROM inserted_bill;

        -- Update debug info with bill ID
        v_debug_info := v_debug_info || format(' | Created bill with ID: %s', v_bill_id);
      EXCEPTION WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS v_error_message = MESSAGE_TEXT,
                              v_stack = PG_EXCEPTION_CONTEXT;
        RAISE EXCEPTION 'Failed to create bill: % (Stack: %) (Debug: %)', 
          v_error_message, v_stack, v_debug_info;
      END;
...
BEGIN
  ...
  -- Note: This is a placeholder. In production, you would use pg_net or similar to make HTTP requests
  -- For now, the PDF will be generated when viewing the bill in the UI
